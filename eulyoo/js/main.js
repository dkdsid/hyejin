...
const book_swiper = new Swiper('.book .swiper', { /* 팝업을 감싼는 요소의 class명 */
    ...
    /*
     스크롤을 내리면 header fixed 클래스를 추가
     스크롤리 맨 위에 있을 때는 fixed 클래스 삭제
          조금이라도 내려오면 fixed 클래스 추가
     브라우저가 스크롤 된 정로를 체크하는 값
     $(window).scrollTop()
     맨 위 0
     스크롤을 내리면 상단에서부터 스크롤 한 만큼 출력

     $(window).scrollTop()rk 0이면 fixed를 삭제
                             0보다 크면 fixed를 추가
     */ 
    ...
    /* 
        ...
        --- 이전에 오버했던 li를 선택하는 명령 x
            모든 li의 on클래스를 다 삭제 
            오버한 li에만ㅁ 다시 on클래스 추가
        <ul>
            <li></li>---오버, 이li 만을 지칭하는 언어  $(this)
            <li></li>
            <li></li>
        /ul>
    */
